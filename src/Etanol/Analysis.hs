    ...
    then (loadedThingsStatus, fDB, mDB)
             ...
             (loadedThingsStatus', fDB', mDB') =
               analysisDriver
                 ...
                 thing
          in trace
               ("LoadedThingsStatus: " ++ (show $ M.size loadedThingsStatus)) $
             analyseAll cmap loadedThings loadedThingsStatus' fDB' mDB'
...
analysisDriver cmap cpool loadedThings loadedThingsStatus fDB mDB thing -- thing is guaranteed to be in loadedThings 
 =
  let thingdata =
        trace ("Here" ++ show (loadedThings !? thing)) $ loadedThings ! thing
   in if isField thing
...
analyseMethod cmap cpool loadedThings loadedThingsStatus fDB mDB thing =
  let mID = methodID thing
      ...
      mNAl =
        map EMethodID $ filter (\x -> M.notMember x mDB) $ map methodID mDeps -- ::[AnyID]
                                 ...
                                 then let desidx = descriptorIndices2 $ mDes
                                          ...
                                          initialState =
                                            AnalysisBundle
                                              ...
                                              [[]]
                                          (finalState, verdict) =
                                            resultant getAnalysis initialState
...
consumeCode = do
  ...
  let q =
        concatMap
          (\(l, s, c, m) -> zip4 (repeat l) (repeat s) (suc g c) (repeat m)) $
        zip4 loc stk p mut
...
getIdxOp (op:rest) =
  if op `elem` (loadsi ++ storesi)
    then toLocalHeapIndex rest
            ...
            | otherwise ->
              error $ "Invalid operator for loading / storing : " ++ show op
 ...
 = do
  ...
  when (op == getStatic) $ do
    ...
    if | ty == OFBasic -> setStackPos j (SBasic : stk)
       ...
       | otherwise ->
         error "Unexpected exception. getStatic does not expect a Reference"
  ...
  when
    (op == iastoreOp || op == fastoreOp || op == lastoreOp || op == dastoreOp) $ do
    let ~(_:(_:(e1:rest))) = stk
        ty = isParamRef e1
    if isJust ty
    ...
    recursiveCalc (c:left) x =
      if c `elem` (basicList ++ longList)
        then if c `elem` longList
               ...
               then (x, SReference x) :
                    recursiveCalc (drop 1 $ dropWhile (';' /=) left) (x + 1)
               else if c == '['
                      then let arrTypeAnd = dropWhile ('[' ==) left
                               arrType =
                                 unsafeHead "descriptorIndices2" arrTypeAnd
                            in if arrType == 'L'
                                 then (x, SReference x) :
                                      recursiveCalc
                                        (drop 1 $ dropWhile (';' /=) arrTypeAnd)
                                        (x + 1)
                                 else (x, SReference x) :
                                      recursiveCalc (drop 1 arrTypeAnd) (x + 1)
                      else []
